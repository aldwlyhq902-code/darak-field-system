...
<head>
    ...
    <style nonce="{{ $cspNonce }}">
        ...
        *{box-sizing:border-box}html{overflow-x:hidden}body{margin:0;background:#f7faf9;color:var(--ink);font-family:Tahoma,"Segoe UI",sans-serif;line-height:1.7;overflow-wrap:anywhere}a{color:var(--teal);text-decoration:none}:focus-visible{outline:3px solid #2dd4bf;outline-offset:3px;border-radius:6px}.skip-link{position:fixed;z-index:100;inset-block-start:8px;inset-inline-start:8px;transform:translateY(-150%);padding:9px 14px;border-radius:9px;background:#fff;color:var(--dark);font-weight:800;box-shadow:0 8px 30px rgba(0,0,0,.2)}.skip-link:focus{transform:none}
        ...
        @media(max-width:700px){.bar{min-height:62px}.nav a{display:none}.grid{grid-template-columns:1fr 1fr}.grid .card:last-child{grid-column:1/-1}.row{grid-template-columns:1fr}.row .btn{width:100%}.hero{padding:22px}.form-card{padding:19px}main{padding-top:18px}input,select,textarea{font-size:16px}}
    </style>
</head>
<body>
...
@auth('client')
<header class="top"><div class="bar"><a class="logo" href="{{ route('client.home') }}" aria-label="دارك — الصفحة الرئيسية">دارك<small>بوابة العميل</small></a><nav class="nav" aria-label="تنقل بوابة العميل"><a href="{{ route('client.home') }}">الرئيسية</a><form method="POST" action="{{ route('client.logout') }}">@csrf<button class="logout">خروج</button></form></nav></div></header>
@endauth
...
<button id="installApp" class="install" type="button">تثبيت بوابة دارك</button>
